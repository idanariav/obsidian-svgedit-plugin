...
const extShadow = {
  ...
  async init() {
    ...
    const toPolar = (dx, dy) => {
      ...
      const angle = length > 0 ? (Math.atan2(dx, -dy) * 180 / Math.PI + 360) % 360 : 150;
      return { angle, length };
    };
    ...
    const setShadow = (params) => {
      ...
      if (params.remove) {
        const filter2 = svgCanvas.getElement(`${elemId}_shadow`);
        const oldFilterAttr = elem.getAttribute("filter");
        if (filter2) {
          batchCmd.addSubCommand(new RemoveElementCommand(filter2, filter2.parentNode));
          filter2.remove();
        }
        if (oldFilterAttr) {
...
export {
  extShadow as default
};
//# sourceMappingURL=ext-shadow.js.map
